...
class UpdateVehicleDetailsController extends GetxController {
  // ⭐ MUST BE NULLABLE TO AVOID DROPDOWN ERRORS
  Rx<VehicleTypeModel?> vehicleTypeModel = Rx<VehicleTypeModel?>(null);

  Rx<VehicleBrandModel?> vehicleBrandModel =
      Rx<VehicleBrandModel?>(null);

  Rx<VehicleModelModel?> vehicleModelModel =
      Rx<VehicleModelModel?>(null);
  ...
  Future<void> onReady() async {
    ...
    if (vehicleTypeList.isNotEmpty) {
      vehicleTypeModel.value = vehicleTypeList.first;
    }
  ...
  void updateData() {
    final uploadCtrl = Get.find<VerifyDocumentsController>();

    if (uploadCtrl.userModel.value.driverVehicleDetails != null) {
      final old = uploadCtrl.userModel.value.driverVehicleDetails!;

      int typeIndex = vehicleTypeList.indexWhere((e) => e.id == old.vehicleTypeId);
      if (typeIndex != -1) vehicleTypeModel.value = vehicleTypeList[typeIndex];

      vehicleBrandController.text = old.brandName ?? '';
      vehicleModelController.text = old.modelName ?? '';
      vehicleNumberController.text = old.vehicleNumber ?? '';
    }
  }
  ...
  Future<void> saveVehicleDetails() async {
    ...
    final verifyCtrl = Get.find<VerifyDocumentsController>();
    ...
    DriverVehicleDetails details = DriverVehicleDetails(
    ...
    userModel.driverVehicleDetails = details;
